...
def __init__(self)
    ...
    distance_label = tk.Label(self.window, text="Distance", font=("Calibri", 18))
    distance_label.pack()
    self.distance_field = tk.Entry(self.window, font=("Consolas", 16))
    self.distance_field.pack()
    time_label = tk.Label(self.window, text="Time", font=("Calibri", 18))
    time_label.pack()
    self.time_field = tk.Entry(self.window, text=("Consolas", 16))
    self.time_field.pack()
    self.speed_label = tk.Label(self.window, text="Answer here...", font=("Calibri", 20))
    self.speed_label.pack()
    button = tk.Button(self.window, text="Calculate Speed", font=("Calibri", 16), command=self.calculate_speed)
    button.pack()
